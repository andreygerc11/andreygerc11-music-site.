...
async def check_mono_payment(amount, comment):
    """Реальна перевірка оплати через Monobank API"""
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            logging.error(f"Mono API error: {e}")
    return False
...
async def verify(callback: types.CallbackQuery):
    ...
    await callback.answer("Перевіряємо виписку...")
    
    if await check_mono_payment(PRICE, track_name):
        url = f"https://drive.google.com/uc?export=download&id={file_id}"
        await callback.message.answer(f"Дякуємо за підтримку! Оплату знайдено. Ваше посилання: {url}")
    else:
        await callback.message.answer("Оплату ще не знайдено. Зачекайте 1-2 хвилини і спробуйте знову.")
