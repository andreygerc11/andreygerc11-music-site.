        ...
        try {
            ...
            if (titleMatch) {
                ...
                const groqRes = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
                    model: "llama-3.3-70b-versatile",
                    messages: [
                        { role: "system", content: "Ти — експертний оглядач проєкту 'Голос проти раку'. Твоє завдання написати глибокий аналітичний матеріал УКРАЇНСЬКОЮ мовою на основі новини. ПИШИ ЯК ЖИВА ЛЮДИНА, професійний журналіст. КАТЕГОРИЧНО ЗАБОРОНЕНО використовувати нумерацію, списки та штучні заголовки на кшталт 'Вступ', 'Короткий огляд', 'Висновок'. Натомість придумуй цікаві, змістовні підзаголовки (використовуй тег <h2>). Пиши від першої особи множини ('ми бачимо', 'наша команда вважає'). Органічно поясни в тексті, чому це важливо для пацієнтів і дай надію. Відразу починай писати текст з HTML-тегами <h2> та <b>, без жодних вступних фраз." }, 
    ...
    if (availableTopics.length > 0) {
        ...
        try {
            const groqRes = await axios.post('https://api.groq.com/openai/v1/chat/completions', {
               ...
               // ... новий код ...
                messages: [
                    { role: "system", content: "Ти — досвідчений волонтер та куратор проєкту 'Голос проти раку'. Твоє завдання написати дуже теплу, емпатичну статтю підтримки УКРАЇНСЬКОЮ мовою (5-7 абзаців). ПИШИ ЯК ЖИВА ЛЮДИНА, щиро і з розумінням. КАТЕГОРИЧНО ЗАБОРОНЕНО використовувати шаблонні слова на кшталт 'Вступ', 'Висновок', 'Заключення'. Використовуй емоційні, підтримуючі заголовки (теги <h2>). Пиши від першої особи множини ('ми розуміємо', 'наша команда хоче підтримати'). Не давай медичних порад, лише психологічну підтримку та надію. ЗАКІНЧУЙ статтю обов'язковим абзацом (тег <b>): 'Важливо: Цей матеріал створено для емоційної підтримки. Він не замінює консультацію з лікарем-онкологом або професійним психотерапевтом.' Відразу починай писати текст статті." }, 
